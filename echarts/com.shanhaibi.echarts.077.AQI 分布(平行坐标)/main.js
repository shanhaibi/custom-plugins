(async function () {
    ...
    let data = {}, schema = [], parallel_axis_data = [];
    let legend_data = [...new Set(value_data[0])];
    if(value_data.length && axis_end_data.length) {
        for(let i = 0; i < value_data[0].length; i++) {
            let row = [];
            if(!data[value_data[0][i]]) data[value_data[0][i]] = [];
            for(let index = 1; index < value_data.length; index++) {
                row.push(value_data[index][i]);
            }
            data[value_data[0][i]].push(row);
        }
        for(let i = 1; i <value_dims.length; i++) {
            schema.push(value_dims[i].alias);
        }
        for(let i = 0; i < axis_end_data.length; i++) {
            if(typeof axis_end_data[i] !== "undefined") {
                parallel_axis_data.push(axis_end_data[i]);
            }
        }
    } else {
        ...
        parallel_axis_data = ['优', '良', '轻度污染', '中度污染', '重度污染', '严重污染'];
        schema = ["日期", "AQI", "PM2.5", "PM10", "CO", "NO2", "SO2", "等级"];
    }
    let series_data = [];
    for(let key of legend_data) {
        series_data.push({
            name: key,
            type: "parallel",
            lineStyle: {
    ...
    let option = {
        ...
        parallel: {
            ...
            parallelAxisDefault: {
                ...
                nameGap: 20,
                nameTextStyle: {
                    color: await ShanhaiBI.getSetting("axis-text-color"),
                    fontSize: 12
                },
                axisLine: {
                    lineStyle: {
